...
type FerieMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function FerieMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: FerieMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = async () => {
    ...
    if (!ids.length) {
      toast.error('Nessuna ferie selezionata')
      return
    }
    ...
    <ConfirmDialog
      ...
      title={
        <span className='text-destructive'>
          ...
          {selectedRows.length > 1 ? 'ferie' : 'ferie'}
        </span>
      }
      desc={
        <div className='space-y-4'>
          <p className='mb-2'>
            Sei sicuro di voler eliminare le ferie selezionate? <br />
