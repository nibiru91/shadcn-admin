...
const formSchema = z.object({
  email: z
    .email({
      error: (iss) =>
        iss.input === '' ? "Inserisci l'email per inviare l'invito." : undefined,
    })
    .optional()
    .or(z.literal('')),
  tag: z.string().min(1, 'Seleziona una tipologia.'),
      ...
      <DialogContent className='sm:max-w-md'>
        <DialogHeader className='text-start'>
          <DialogTitle className='flex items-center gap-2'>
            <MailPlus /> Invita azienda
          </DialogTitle>
          <DialogDescription>
            Invia un invito email a un referente aziendale e specifica la
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      type='email'
                      placeholder='es: contatti@azienda.it'
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
