...
type CommesseMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function CommesseMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: CommesseMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = async () => {
    ...
    toast.promise(promise, {
      loading: 'Eliminazione commesse...',
      success: async () => {
        await queryClient.invalidateQueries({ queryKey: ['commesse'] })
        table.resetRowSelection()
        onOpenChange(false)
        return `Eliminate ${ids.length} commess${ids.length === 1 ? 'a' : 'e'}`
      },
    ...
    <ConfirmDialog
      ...
      title={
        <span className='text-destructive'>
          ...
          {selectedRows.length > 1 ? 'commesse' : 'commessa'}
        </span>
      }
      desc={
        <div className='space-y-4'>
          <p className='mb-2'>
            Sei sicuro di voler eliminare le commesse selezionate? <br />
